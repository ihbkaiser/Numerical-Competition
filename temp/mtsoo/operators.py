...
def find_relative(population, skill_factor, sf, N, nums, except_idx = None):

  """
+  This function finds and returns a subset of the population based on the skill factor and other parameters.
+
+  Parameters:
+      population (array-like): The population from which to select the subset.
+      skill_factor (array-like): The skill factor corresponding to each individual in the population.
+      sf (int): The specific skill factor to filter the population by.
+      N (int): The maximum index to consider in the skill_factor array.
+      nums (int): The number of elements to select from the population.
+      except_idx (int, optional): The index to exclude from the selection.
+
+  Returns:
+      array-like: A subset of the population based on the skill factor and other parameters.
+  """

  if except_idx is None:
   return population[np.random.choice(np.where(skill_factor[:N] == sf)[0], nums)]
  else:
    print(np.where(skill_factor[:N] == sf)[0])
    idx = np.delete(np.where(skill_factor[:N] == sf)[0], except_idx)
    return population[np.random.choice(idx, nums)]


def find_non_relative(population, skill_factor, sf, N, nums):
  return population[np.random.choice(np.where(skill_factor[:N] != sf)[0], nums)]
...
def get_not_subpops(population, skill_factor, sf):
