# Reference : https://github.com/P-N-Suganthan/2022-SO-BO

# # import numpy as np
# from CECFuncs import *
# -*- coding: utf-8 -*-
"""
...
def cec22_test_func(x, nx, mx, func_num):
  ...
  if ini_flag == 0:
    ...
    if (nx!=2 and nx!=10 and nx!=20):
      raise ValueError(f"\033[91m\nError: Test functions are only defined for D=2,10,20, you have D={nx}.\n\033[0m")

    if (nx==2) and (func_num==6 or func_num==7 or func_num==8):
      raise ValueError(f"\033[91m\nError: For function {func_num}, D=2 is not defined. Try D=10, 20\n\033[0m")
      
    
    # Load M matrix
    
    FileName = r'C:\Users\DELL\OneDrive - Hanoi University of Science and Technology\Documents\Numerical Competition\src\cec2022\M_%d_D%d.txt'%(func_num, nx)
    try:
...
#      print("\nError: there is insufficient memory available!\n")
    ...
    # Shift data
    FileName = r"C:\Users\DELL\OneDrive - Hanoi University of Science and Technology\Documents\Numerical Competition\src\cec2022\shift_data_%d.txt"%func_num
...
#      print("\nError: there is insufficient memory available!\n")
    ...
    if (func_num >= 6) & (func_num <=8):
        FileName = r"C:\Users\DELL\OneDrive - Hanoi University of Science and Technology\Documents\Numerical Competition\src\cec2022\shuffle_data_%d_D%d.txt" %(func_num, nx)
        try:
...
class CEC2022:
    def __init__(self, i, dim):
        if i < 1 or i > 12:
            raise ValueError("Function number must be between 1 and 12")
        self.idx = i
        self.dim = dim
        self.LB = -100
        self.UB = 100
        self.opt_list = [300, 400, 600, 800, 900, 1800, 2000, 2200, 2300, 2400, 2600, 2700]
        self.opt_value = self.opt_list[i-1]
        self.best_val = np.inf
        self.FE = 0
        self.aceps = np.inf
        self.max_FE = 200000 if dim == 10 else 1000000
        self.CEC_object = cec2022_func(func_num = i)
    def fitness_of_ind(self, X):
        self.FE = self.FE + 1
        # print(X)
        # print('type: ', type(X))
        # print('shape: ', np.shape(X))
        assert np.shape(X)[0] == self.dim, f'Dimension of your input vector is {np.shape(X)[0]} but it should be {self.dim} !'
        X_cec = np.array(X).reshape(self.dim, 1)
        result = self.CEC_object.values(X_cec).ObjFunc[0]
        self.best_val = min(self.best_val, result)
        if abs(result - self.opt_value) < 1e-8 and np.isinf(self.aceps):
            self.aceps = self.FE
        return result
    def fitness_mfea(self, X):
        assert np.shape(X)[0] == self.dim, f'Dimension of your input vector is {np.shape(X)[1]} but it should be {self.dim} !'
        X_modified = self.LB + np.array(X)*(self.UB-self.LB)
        return self.fitness_of_ind(X_modified)
...
# x = 200.0*np.random.rand(nx, 1)-100.0
# F = CEC.values(x)
# print(F.ObjFunc)